function setAuthCredentials(token, userId, username, userType) {
    localStorage.setItem('auth-token', token);
    localStorage.setItem('auth-user', username);
    localStorage.setItem('auth-user-id', userId);
    localStorage.setItem('auth-user-type', userType);  // Neu: Typ speichern
}
...
function removeAuthCredentials() {
    ...
    localStorage.removeItem('auth-user-type');  // Neu: Typ auch entfernen
}
...
function getAuthUserType() {
    return localStorage.getItem('auth-user-type');
}

function getProfileUrl(userId) {
    const userType = getAuthUserType();
    if (userType === 'business') {
        return `business-profile/user/${userId}/`;
    } else {
        return `customer-profile/user/${userId}/`;
    }
}
